...
export default function EncoursEngagementsPage() {
  ...
  const fetchDataByDate = async (date: string) => {
    ...
    const personnesElements = xmlDoc.querySelectorAll("EncoursEngagement");
    const mapped = Array.from(personnesElements).map(el => ({
      id: parseInt(el.getAttribute("IdInterneClt") || "0"),
      NatDec: parseInt(el.getAttribute("NatDec") || "0"),
      RefIntEng: el.getAttribute("RefIntEng") || "",
      CodDev: el.getAttribute("CodDev") || "",
      MntDerEch: el.getAttribute("MntDerEch") || "",
      MonPai: el.getAttribute("MonPai") || "",
      MntHBil: el.getAttribute("MntHBil") || "",
      MntRemAnt: el.getAttribute("MntRemAnt") || "",
      MntCRDU: el.getAttribute("MntCRDU") || "",
      MntCreRat: el.getAttribute("MntCreRat") || "",
      NbrEchPay: el.getAttribute("NbrEchPay") || "",
    }));
    setEncoursEngagements(mapped);
    ...
    <div className="min-h-screen bg-[#f3f6fb] text-slate-800">
      ...
      >
        ...
        <div className="flex-1 overflow-auto px-4 pb-10 pt-6 sm:px-6">
          <div className="mb-6">
            <h1 className="text-lg font-semibold text-slate-800">Liste des données en cours d'engagements</h1>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-slate-50">
                <tr className="text-left text-xs uppercase tracking-wide text-slate-500">
                  ...
                  <th className="w-28 px-3 py-2">NatDec</th>
                  <th className="w-28 px-3 py-2">RefIntEng</th>
                  <th className="px-3 py-2">CodDev</th>
                  <th className="px-3 py-2">MntDerEch</th>
                  <th className="w-20 px-3 py-2">MonPai</th>
                  <th className="w-32 px-3 py-2">MntHBil</th>
                  <th className="px-3 py-2">MntCRDU</th>
                  <th className="px-3 py-2">MntCreRat</th>
                  <th className="px-3 py-2">NbrEchPay</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
